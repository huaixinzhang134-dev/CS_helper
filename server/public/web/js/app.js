      ...
      <div style="text-align:center;padding:30px 0;">
        ...
        <div style="margin-bottom:24px;">
          ...
          <div style="display:flex;gap:8px;justify-content:center;">
            ...
            <button class="btn ${state.mode==='friend'?'btn-success':'btn-ghost'}" onclick="App.requireLogin(()=>{App.state.guess.mode='friend';App.renderGuess(document.getElementById('pageContent'))})">好友PK</button>
          </div>
        </div>
        ${state.mode ? `
        <div>
...
选手位置包括：步枪手、狙击手、教练`);
  ...
  async renderUser(container) {
    if (!this.user) {
      container.innerHTML = `
        ...
        <div class="user-header" style="flex-direction:column;text-align:center;padding:40px;">
          ...
          <h3 style="margin:12px 0 8px;">未登录</h3>
          <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;">登录后同步游戏数据</p>
          <button class="btn" onclick="App.showLogin()">手机号登录</button>
        </div>
      `;
      return;
    }
    ...
    try {
      ...
      const user = profileRes.code === 0 ? profileRes.data : this.user;
      ...
      container.innerHTML = `
        ...
        <div class="user-header">
          <div class="user-avatar-placeholder">👤</div>
          <div style="flex:1;">
            <h3>${esc(user.nickname||'未设置昵称')}</h3>
            <div class="user-stats">
              <div class="user-stat"><span class="user-stat-num">${user.winCount||0}</span><span class="user-stat-label">胜场</span></div>
              <div class="user-stat"><span class="user-stat-num">${user.totalGames||0}</span><span class="user-stat-label">总局</span></div>
              <div class="user-stat"><span class="user-stat-num">${user.winRate||0}%</span><span class="user-stat-label">胜率</span></div>
            </div>
          </div>
        </div>
        <div class="card" onclick="App.goTo('shop')" style="display:flex;align-items:center;gap:12px;cursor:pointer;">
          <span style="font-size:24px;">🪙</span>
          <div style="flex:1;">
            <div style="font-size:13px;color:var(--text-muted);">我的代币</div>
            <div style="font-size:20px;font-weight:700;">${user.coins||0}</div>
          </div>
          <span style="color:var(--text-muted);">›</span>
        </div>
        <div class="menu-list" style="margin-top:12px;">
          <div class="menu-item" onclick="App._showGuessRecords()">
            <span class="menu-item-text">📊 我的竞猜记录</span>
            <span class="menu-item-arrow">›</span>
          </div>
          <div class="menu-item" onclick="App._showRankingModal()">
            <span class="menu-item-text">🏆 查看封神榜</span>
            <span class="menu-item-arrow">›</span>
          </div>
          <div class="menu-spacer"></div>
          <div class="menu-item" onclick="App.goTo('picks')">
            <span class="menu-item-text">🏆 年度 Top30 猜测</span>
            <span class="menu-item-arrow">›</span>
          </div>
          <div class="menu-item" onclick="App.goTo('shop')">
            <span class="menu-item-text">🛒 道具商城</span>
            <span class="menu-item-arrow">›</span>
          </div>
        ...
        <div class="pick-grid">
          ${slots.map(s => `
            <div class="pick-slot ${s.playerName?'filled':''} ${!s.canSubmit?'disabled':''}"
                 onclick="App._openPickSlot(${s.slot},${JSON.stringify(s).replace(/"/g,"'")})">
  ...
  _openPickSlot(slot, slotData) {
    if (!slotData.canSubmit) { alert('提交时间已过，不可提交'); return; }
    if (slotData.submissionNo >= slotData.maxSubmissions) { alert(`已达最大提交次数 ${slotData.maxSubmissions}`); return; }
    ...
    const html = `<div class="modal-mask" id="pickModal">
      <div class="search-panel" onclick="event.stopPropagation()">
        ...
        <div style="padding:0 16px;font-size:12px;color:var(--text-muted);">
          当前提交: 第 ${slotData.submissionNo + 1} / ${slotData.maxSubmissions} 次
        </div>
  ...
  _adminSwitchTab(tab) {
    this._adminTab = tab;
    document.querySelectorAll('#adminTabContent .tab-item').forEach(el => el.classList.toggle('active', el.dataset.tab === tab));
