 ...
 *
 * 判定规则（每个大小写不敏感的重名组）：
 *   保留 —— 名字与 nicknames.json 大小写精确一致的选手（官方目标）；
 *            或有 status='approved' 的 nickname_suggestions 的选手（合法审核目标）。
 *   清空 —— 其余选手（其别名只可能来自旧版批量导入的误刷，审核记录里查不到对应建议）。
...
async function main() {
  ...
  for (const [key, rows] of groups) {
    ...
    // 保留理由：与 nicknames.json 大小写精确一致（官方目标）或有过审核通过记录
    const keep = rows.filter(r => jsonPlayerNames.has(r.name) || approvedIds.has(String(r.id)));
    const victims = rows.filter(r => !keep.includes(r));
    ...
    console.log(`\n== 重名组 "${key}"（${rows.length} 人）`);
    console.log(`   保留（官方/已审核）:`);
    keep.forEach(r => console.log(`    ✓ ${fmt(r)}`));
    if (!victims.length) {
      console.log('   无需要清理的行');
      continue;
    }
